...
// --- CONFIGURACI├ôN DE ICONOS ---
// Componente para manejar iconos de Lucide v├¡a CDN
...
function App() {
  ...
  // 1. Cargar datos iniciales desde el servidor y sincronizaci├│n peri├│dica
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Sincronizar localmente para otras pesta├▒as
  ...
  // 2. Sincronizaci├│n entre Pesta├▒as (Pilar 3)
  useEffect(() => {
  ...
  const switchCategory = (newCat) => {
    ...
    showToast(`Cambiado a ${newCat === 'quest' ? 'Robotics Quest' : 'Seguidor de L├¡nea'}`);
  };
  ...
  const addTeam = (teamData) => {
    ...
    postTeams(updated);
    showToast('Equipo registrado con ├®xito');
  };
  ...
  const disqualifyTeam = (teamId, reason) => {
    setConfirmDialog({
      message: `┬┐Est├ís seguro de descalificar al equipo por: ${reason}?`,
      onConfirm: () => {
  ...
  const deleteTeam = (id) => {
    ...
    if (!team) return;
    setConfirmDialog({
      message: `┬┐Est├ís seguro de eliminar permanentemente al equipo "${team.teamName || team.school}"?`,
      onConfirm: () => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl">
            <div className="flex items-center gap-3 text-red-600 mb-4">
              ...
              <h3 className="font-black text-xl">Atenci├│n</h3>
            </div>
      ...
      <nav className="bg-blue-950 text-white md:w-64 flex-shrink-0 flex md:flex-col shadow-2xl z-20 sticky top-0 md:h-screen">
          ...
          <div className="bg-blue-900/50 p-3 rounded-xl">
            ...
            {currentUser.role === 'admin' && (
              <div className="mt-3 pt-3 border-t border-blue-800/50">
                <p className="text-[8px] font-black text-blue-400 uppercase tracking-widest mb-2 text-center">Cambiar Categor├¡a</p>
        ...
        <div className="flex flex-1 md:flex-col overflow-x-auto md:overflow-y-auto">
          {currentUser.role === 'admin' && (
            <>
              ...
              <NavButton active={activeTab === 'inspeccion'} onClick={() => setActiveTab('inspeccion')} icon={<Icon name="clipboard-check" />} label="Inspecci├│n" />
          ...
          <NavButton active={activeTab === 'evaluacion'} onClick={() => setActiveTab('evaluacion')} icon={<Icon name="play-circle" />} label="Evaluaci├│n" />
          <NavButton active={activeTab === 'resultados'} onClick={() => setActiveTab('resultados')} icon={<Icon name="trophy" />} label="Resultados" />
        </div>

        {/* Secci├│n Inferior de la Sidebar */}
        <div className="p-4 border-t border-blue-900 space-y-3">
          ...
          >
            ...
            <span className="text-[10px] font-black uppercase tracking-widest">Cerrar Sesi├│n</span>
          </button>
        </div>
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-fadeIn border border-slate-700">
        <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-10 text-center text-white">
          ...
          <p className="text-blue-100 font-bold mt-2 uppercase text-[10px] tracking-widest leading-none">Plataforma de Competencia de Rob├│tica</p>
        </div>
        <form onSubmit={handleLogin} className="p-8 space-y-4">
          <div>
            <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2 block">Categor├¡a de Competencia</label>
            <div className="relative">
              ...
              >
                ...
                <option value="line_follower">Seguidor de L├¡nea (Velocidad/Porcentaje)</option>
              </select>
          ...
          <div>
            <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2 block">Contrase├▒a</label>
            <div className="relative">
              <input
                type="password"
                value={password}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-fadeIn">
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl">
        <div className="bg-slate-50 p-8 border-b border-slate-100 flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black text-blue-500 uppercase tracking-[0.2em] mb-1">Auditor├¡a de Desempe├▒o</p>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          ...
          <div className="flex gap-2 p-1 bg-slate-100 rounded-2xl w-full lg:w-auto">
            ...
            <button onClick={() => setMode('obstacle')} className={`flex-1 lg:flex-none px-6 py-3 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all ${mode === 'obstacle' ? 'bg-white text-red-600 shadow-md' : 'text-slate-500 hover:bg-slate-200'}`}>
              <Icon name="x-circle" className="w-3 h-3 text-red-600" /> OBST├üCULO
            </button>
          </div>
        </div>
          ...
          <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 h-fit">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-red-100">
                <p className="text-[10px] font-bold text-red-400 mb-1 uppercase">Obst├ículos</p>
...
function UsuariosTab({ users, fetchUsers, showToast, setConfirmDialog }) {
  ...
  const handleDeleteUser = (userId) => {
    ...
    setConfirmDialog({
      message: `┬┐Est├ís seguro de eliminar al usuario ${userId}?`,
      onConfirm: async () => {
        try {
    ...
    <div className="max-w-4xl mx-auto animate-fadeIn space-y-8">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-200">
        <h2 className="text-2xl font-black text-blue-900 mb-6 uppercase italic flex items-center gap-3">
          <Icon name="user-plus" className="text-blue-600" /> A├▒adir Nuevo Juez
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Contrase├▒a" className="p-4 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold outline-none focus:border-blue-500" />
        </div>
...
function RegistroTab({ addTeam }) {
  ...
  const handleAdd = () => {
    console.log("Intentando a├▒adir equipo:", { schoolName, teamName, memberCount, members });
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-1">
              ...
              <input value={schoolName} onChange={e => setSchoolName(e.target.value)} className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold outline-none focus:border-blue-500 text-lg" placeholder="Nombre de la Instituci├│n" />
            </div>
            <div className="md:col-span-1">
              <label className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-2 block">Nombre del Equipo</label>
              <input value={teamName} onChange={e => setTeamName(e.target.value)} className="w-full p-4 rounded-2xl bg-slate-50 border-2 border-slate-100 font-bold outline-none focus:border-blue-500 text-lg" placeholder="Nombre ├║nico del equipo" />
            </div>
            <div className="md:col-span-2">
              <label className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-2 block">N┬║ Integrantes (M├íx 3)</label>
            ...
            {members.slice(0, memberCount).map((member, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                ...
                <div>
                  ...
                  <input value={member.surname} onChange={e => handleMemberChange(idx, 'surname', e.target.value)} className="w-full p-3 rounded-xl bg-white border border-slate-200 font-bold outline-none focus:border-blue-500 text-sm" placeholder="Ej: P├®rez" />
                </div>
              </div>
            ))}
          </div>
...
function InspeccionTab({ teams, updateTeamStatus, disqualifyTeam }) {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fadeIn">
      <h2 className="text-3xl font-black text-blue-900 mb-8 uppercase italic">Inspecci├│n de Hardware</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {pending.length === 0 && (
...
function ResultadosTab({ teams, currentUser, onShowHistory, deleteTeam }) {
  const sorted = useMemo(() => {
    ...
    if (currentUser.category === 'line_follower') {
      return [...list].sort((a, b) => {
        ...
        // Factor 2: Tiempo (Menor a Mayor) - Usamos el ├║ltimo tiempo registrado
  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto">
      <h2 className="text-4xl font-black text-blue-900 tracking-tighter uppercase italic">Ranking {currentUser.category === 'line_follower' ? 'Seguidor de L├¡nea' : 'Global'}</h2>
      <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-blue-600 text-white">
            <tr>
              <th className="p-6 text-[10px] uppercase w-24 text-center">Puesto</th>
              <th className="p-6 text-[10px] uppercase">Instituci├│n</th>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-6 border-t border-slate-100 flex gap-4">
                <button onClick={() => setPenalties(p => p + 1)} className="flex-1 bg-orange-50 hover:bg-orange-100 text-orange-600 p-4 rounded-2xl border-2 border-orange-100 font-black text-xs flex flex-col items-center gap-1 transition-all">
                  <Icon name="alert-triangle" /> Penalizaci├│n (+5s)
              ...
              <button onClick={handleSave} disabled={!selTeam || !canAttempt} className="w-full bg-green-500 hover:bg-green-600 disabled:bg-slate-200 text-white font-black py-5 rounded-2xl shadow-xl uppercase tracking-widest transition-all">
                {selTeam && !canAttempt ? 'L├¡mite de Intentos Alcanzado' : 'Guardar Resultado'}
              </button>
            </div>
          </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-blue-900 uppercase italic">Mapa de Evaluaci├│n - Pista {selPista}</h3>
            ...
            <div className="mt-6 flex items-center gap-4 p-5 bg-blue-50 rounded-[2rem] border border-blue-100">
              ...
              <p className="text-xs text-blue-900 font-bold">Instrucciones: Pulsa sobre cada pieza para marcarla como completada. El sistema calcula autom├íticamente el puntaje y el porcentaje de recorrido para el ranking.</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-blue-400 font-bold uppercase tracking-[0.3em] text-sm mt-2">
              {category === 'line_follower' ? 'Seguidor de L├¡nea' : 'Robotics Quest'}
            </p>
          </div>
        </div>
        ...
        {sorted.map((t, i) => (
          <div key={t.id} className={`flex items-center gap-6 p-6 rounded-3xl border-2 transition-all ${i === 0 ? 'bg-blue-600/20 border-blue-500 transform scale-[1.02] shadow-2xl' : 'bg-slate-900/50 border-slate-800'} ${t.status === 'disqualified' ? 'opacity-30' : ''}`}>
            <div className="w-20 text-center flex flex-col items-center">
              ...
              <span className="text-[10px] font-bold text-slate-500 mt-1 uppercase tracking-widest">{i === 0 ? 'L├¡der' : ''}</span>
            </div>
