...
function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      // GUARDIA DE CONTINGENCIA: No sobreescribir datos si hay un guardado activo
  ...
  const postTeams = async (newTeams) => {
    setTeams(newTeams);
    localStorage.setItem('ada_teams', JSON.stringify(newTeams));

    // Activar indicador de guardado y guardia anti-sobreescritura
    setIsSaving(true);
    setSaveError(false);
    ...
    try {
      ...
      console.error("Error guardando datos en servidor:", err);
      setSaveError(true);
      showToast('⚠️ Error al guardar en servidor. Los datos están seguros localmente.');
    } finally {
      setIsSaving(false);
      // Liberar guardia después de 2s para que el poll no sobreescriba datos recién guardados
  ...
  // handleUpdateTeamBase acepta un objeto patch para soportar el Modo Contingencia
  // Uso normal:      handleUpdateTeamBase(id, name, school)
  // Modo Contingencia: handleUpdateTeamBase(id, name, school, { newHistoryEntries: [...] })
  const handleUpdateTeamBase = (teamId, newTeamName, newSchoolName, extraPatch = {}) => {
    const updated = teams.map(t => {
      if (t.id !== teamId) return t;

      let newHistory = t.history;

      // CONTINGENCIA: Si hay entradas por ronda, las agrega al historial
      if (extraPatch.newHistoryEntries && extraPatch.newHistoryEntries.length > 0) {
      ...
      return {
        ...
        history: newHistory,
        score: newScore,
        lastTime: newLastTime > 0 ? newLastTime : t.lastTime,
      };
    });
    setTeams(updated);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans text-slate-800 relative">
      ...
      <nav className="bg-blue-950 text-white md:w-64 flex-shrink-0 flex flex-col shadow-2xl z-20 sticky top-0 md:h-screen w-full md:max-h-screen overflow-hidden">
        ...
        <div className="p-4 border-t border-blue-900 hidden md:flex items-center gap-2">
          {isSaving ? (
            <>
              <div className="w-2 h-2 rounded-full bg-yellow-400 animate-pulse shadow-[0_0_10px_rgba(250,204,21,0.6)]"></div>
              <span className="text-[10px] font-bold text-yellow-300 uppercase tracking-widest">Guardando...</span>
            </>
          ) : saveError ? (
            <>
              <div className="w-2 h-2 rounded-full bg-red-400 shadow-[0_0_10px_rgba(248,113,113,0.6)]"></div>
              <span className="text-[10px] font-bold text-red-300 uppercase tracking-widest">Error de Red</span>
            </>
          ) : (
            <>
...
function NavButton({ active, onClick, icon, label }) {
  return (
    <button onClick={onClick} className={`flex-shrink-0 md:flex-none flex flex-col md:flex-row items-center gap-1 md:gap-3 p-3 md:p-4 transition-all duration-200 border-b-2 md:border-b-0 md:border-l-4 ${active ? 'bg-blue-600/20 md:bg-blue-600 text-white border-blue-500 shadow-lg' : 'text-blue-300/50 border-transparent hover:bg-blue-900 hover:text-white'
      }`}>
...
function EvaluacionTab({ teams, tracks, addScore, currentUser, disqualifyTeam, postTeams, showToast, timer, competitionDuration, onUpdateTeamBase, onDeleteTeam }) {
  ...
  const existingEvaluation = useMemo(() => {
    ...
    // Cada ronda es independiente: buscamos si evaluó ESTA pista en ESTA ronda
  ...
  const evalRemaining = useMemo(() => {
    if (!selectedTeamData) return true;
    // 1 ticket por pista por ronda
    const roundEval = selectedTeamData.history?.find(h => h.ronda === selRonda && h.pista === selPista && h.practice !== true);
    return !roundEval;
  }, [selectedTeamData, selRonda, selPista]);
    ...
    <div className="space-y-6 animate-fadeIn">
              ...
              {existingEvaluation && (
                <div className="bg-orange-50 border border-orange-200 p-4 rounded-2xl flex gap-3 items-start animate-fadeIn">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-orange-800 font-medium">Este equipo ya fue evaluado en la <strong>Ronda {existingEvaluation.ronda}</strong> por <strong>{existingEvaluation.judgeName}</strong>.</p>
                  </div>
                </div>
              )}

              {(!existingEvaluation && !evalRemaining) && (
                <div className="bg-red-50 border border-red-200 p-4 rounded-2xl flex gap-3 items-start animate-fadeIn">
                  <Icon name="alert-circle" className="text-red-500 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-[10px] font-black text-red-600 uppercase tracking-widest">Pista Agotada</p>
                    <p className="text-xs text-red-800 font-medium">Este equipo ya consumió su oportunidad de evaluación para esta pista.</p>
                  </div>
                </div>
              )}
      ...
      {showEditTeam && selTeam && teams && (
        <EditTeamModal
          ...
          onSave={(name, school, extraPatch) => {
            if (onUpdateTeamBase) onUpdateTeamBase(selTeam, name, school, extraPatch);
...
function InspeccionTab({ teams, updateTeamStatus, disqualifyTeam, onUpdateTeamBase, onDeleteTeam, postTeams, showToast, questDuration, lineDuration }) {
  const [editingTeam, setEditingTeam] = useState(null);
  const [contingencyMode, setContingencyMode] = useState(false);
  const pending = teams.filter(t => t.status === 'pending');

  // Obtener duración total según categoría del equipo en edición
  const totalDurationMin = editingTeam ? (editingTeam.category === 'quest' ? questDuration : lineDuration) : 25;
  return (
    <div className="max-w-4xl mx-auto animate-fadeIn">
      {/* CABECERA CON TOGGLE CONTINGENCIA */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
        <h2 className="text-2xl md:text-3xl font-black text-blue-900 uppercase italic leading-tight">Inspección de Hardware</h2>
        <button
          onClick={() => setContingencyMode(!contingencyMode)}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest border-2 transition-all flex-shrink-0 ${contingencyMode ? 'bg-orange-500 border-orange-400 text-white shadow-lg shadow-orange-500/30 animate-pulse' : 'bg-white border-orange-200 text-orange-600 hover:bg-orange-50'}`}
        >
          <Icon name="shield-alert" className="w-4 h-4" />
          {contingencyMode ? '⚡ Modo Contingencia ACTIVO' : 'Activar Modo Contingencia'}
        </button>
      </div>

      {contingencyMode && (
        <div className="mb-6 bg-orange-50 border-2 border-orange-300 p-4 rounded-2xl flex gap-3 items-start animate-fadeIn">
          <Icon name="alert-triangle" className="text-orange-500 flex-shrink-0 mt-0.5 w-5 h-5" />
          <div>
            <p className="text-[10px] font-black text-orange-700 uppercase tracking-widest">Modo de Emergencia Activo</p>
            <p className="text-xs text-orange-800 font-medium mt-1">Haz clic en el ícono de edición de cualquier equipo para sobreescribir su puntaje y tiempo manualmente. Solo usa esto si el sistema perdió datos y los tienes en papel.</p>
          </div>
        </div>
      )}
      ...
      {editingTeam && (
        <EditTeamModal
          ...
          onSave={(name, school, extraPatch) => {
            onUpdateTeamBase(editingTeam.id, name, school, extraPatch);
            setEditingTeam(null);
          }}
          onDelete={() => {
            setEditingTeam(null);
...
function EditTeamModal({ team, onClose, onSave, onDelete, isContingencyMode, totalDurationMin = 25 }) {
  ...
  // ── Estado del formulario de entrada por ronda ──
  const [selRonda, setSelRonda] = useState(1);
  const [selPista, setSelPista] = useState(1);
  const [selPoints, setSelPoints] = useState(0);

  // TIEMPO RESTANTE (Cronómetro inverso)
  const [selRemainMin, setSelRemainMin] = useState(totalDurationMin);
  const [selRemainSec, setSelRemainSec] = useState(0);

  const [pendingEntries, setPendingEntries] = useState([]);

  // Historial oficial actual (no prácticas)
  const officialHistory = (team.history || []).filter(h => !h.practice);

  const addPendingEntry = () => {
    if (Number(selPoints) < 0) return;
    const rondaNum = Number(selRonda);
    const pistaNum = Number(selPista);

    // Cálculo de Tiempo Transcurrido: Total - Restante
    const totalSec = totalDurationMin * 60;
    const remainSec = (Number(selRemainMin) * 60) + Number(selRemainSec);
    const elapsedMs = Math.max(0, (totalSec - remainSec) * 1000);

    // Detectar duplicado en pendientes
    const alreadyPending = pendingEntries.find(e => e.ronda === rondaNum && e.pista === pistaNum);
    if (alreadyPending) {
      // Reemplazar en lugar de duplicar
      setPendingEntries(prev => prev.map(e =>
        e.ronda === rondaNum && e.pista === pistaNum
          ? { ...e, points: Number(selPoints), finalTimeMs: elapsedMs, finalTime: elapsedMs }
          : e
      ));
    } else {
      setPendingEntries(prev => [...prev, {
        ronda: rondaNum,
        pista: pistaNum,
        points: Number(selPoints),
        finalTimeMs: elapsedMs,
        finalTime: elapsedMs,
        practice: false,
        convertedFromPractice: false,
        date: new Date().toLocaleTimeString(),
        judgeId: 'contingencia',
        judgeName: 'Modo Contingencia',
      }]);
    }
    setSelPoints(0);
    // Resetear a tiempo completo (0 transcurrido)
    setSelRemainMin(totalDurationMin);
    setSelRemainSec(0);
  };

  const removePending = (i) => setPendingEntries(prev => prev.filter((_, idx) => idx !== i));

  const pendingTotal = pendingEntries.reduce((s, e) => s + e.points, 0);
  const currentTotal = officialHistory.reduce((s, h) => s + (h.points || h.percentage || 0), 0);

  const handleSave = () => {
    const extraPatch = {};
    if (isContingencyMode && showContingency && pendingEntries.length > 0) {
      extraPatch.newHistoryEntries = pendingEntries;
    }
    onSave(name, school, extraPatch);
  };

  const rondaLabel = (r) => r === 5 ? 'Gran Final' : `Ronda ${r}`;

  return (
    <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-white max-w-md w-full rounded-[2rem] shadow-2xl p-6 transform animate-fadeIn border-2 border-blue-500 my-4">
        <h3 className="text-xl font-black text-center uppercase text-slate-800 mb-5">Editar Equipo</h3>

        <div className="space-y-3 mb-5">
          <div>
            <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest block mb-1.5">Nombre del Equipo</label>
            <input value={name} onChange={e => setName(e.target.value)} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 font-bold focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
          <div>
            <label className="text-[10px] font-black text-blue-400 uppercase tracking-widest block mb-1.5">Institución / Colegio</label>
            <input value={school} onChange={e => setSchool(e.target.value)} className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 font-bold focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
        </div>

        {/* ── MODO CONTINGENCIA POR RONDA ── */}
        {isContingencyMode && (
          <div className="mb-5">
            <button
              onClick={() => setShowContingency(!showContingency)}
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest border-2 transition-all ${showContingency ? 'bg-orange-500 border-orange-400 text-white shadow-lg shadow-orange-500/30' : 'bg-orange-50 border-orange-200 text-orange-600 hover:bg-orange-100'}`}
            >
              <span className="flex items-center gap-2">
                <Icon name="shield-alert" className="w-4 h-4" />
                Modo Contingencia (por Ronda)
              </span>
              <Icon name={showContingency ? 'chevron-up' : 'chevron-down'} className="w-4 h-4" />
            </button>
            ...
            {showContingency && (
              <div className="mt-3 space-y-4 animate-fadeIn">
                ...
                <div className="bg-slate-800 p-3 rounded-xl flex justify-between items-center">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Puntaje Actual en Sistema</span>
                  <span className="text-2xl font-black text-white">{currentTotal} <span className="text-xs text-slate-400">pts</span></span>
                </div>

                {/* Historial oficial existente */}
                {officialHistory.length > 0 && (
                  <div className="bg-slate-50 border border-slate-200 p-3 rounded-xl">
                    <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2">Evaluaciones Registradas</p>
                    <div className="space-y-1.5 max-h-28 overflow-y-auto">
                      {officialHistory.map((h, i) => (
                        <div key={i} className="flex justify-between items-center text-xs font-bold text-slate-600 bg-white px-3 py-1.5 rounded-lg border border-slate-100">
                          <span>{rondaLabel(h.ronda)} · {h.pista === 0 ? 'Global' : `Pista ${h.pista}`}</span>
                          <span className="text-blue-600 font-black">{h.points || h.percentage || 0} pts</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-orange-50 border-2 border-orange-200 p-4 rounded-2xl space-y-3">
                  <p className="text-[9px] font-black text-orange-700 uppercase tracking-widest">Agregar Evaluación Manual</p>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[9px] font-black text-orange-600 uppercase tracking-widest block mb-1">Ronda</label>
                      <select value={selRonda} onChange={e => setSelRonda(e.target.value)} className="w-full p-2.5 rounded-xl bg-white border-2 border-orange-200 font-bold text-sm focus:outline-none focus:ring-2 focus:ring-orange-400">
                        {[1, 2, 3, 4].map(r => <option key={r} value={r}>Ronda {r}</option>)}
                        <option value={5}>Gran Final</option>
                      </select>
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[9px] font-black text-orange-600 uppercase tracking-widest block mb-1">Puntos</label>
                      <input
                        type="number" min="0"
                        value={selPoints}
                        onChange={e => setSelPoints(e.target.value)}
                        className="w-full p-2.5 rounded-xl bg-white border-2 border-orange-300 font-black text-xl text-orange-800 text-center focus:outline-none focus:ring-2 focus:ring-orange-400"
                      />
                    </div>
                    <div>
                      <label className="text-[9px] font-black text-orange-600 uppercase tracking-widest block mb-1">T. Restante (mm:ss)</label>
                      <div className="flex items-center gap-1">
                        <input
                          type="number" min="0" max={totalDurationMin}
                          value={selRemainMin}
                          onChange={e => setSelRemainMin(e.target.value)}
                          className="w-full p-2.5 rounded-xl bg-white border-2 border-orange-200 font-bold text-center focus:outline-none focus:ring-2 focus:ring-orange-400"
                        />
                        <span className="font-bold text-orange-400">:</span>
                        <input
                          type="number" min="0" max="59"
                          value={selRemainSec}
                          onChange={e => setSelRemainSec(e.target.value)}
                          className="w-full p-2.5 rounded-xl bg-white border-2 border-orange-200 font-bold text-center focus:outline-none focus:ring-2 focus:ring-orange-400"
                        />
                      </div>
                    </div>
                  ...
                  {/* Preview del cálculo de tiempo */}
                  <div className="bg-orange-100/50 p-2 rounded-lg border border-orange-200 flex justify-between items-center px-4">
                    <span className="text-[8px] font-black text-orange-600 uppercase">Tiempo Transcurrido (Calculado):</span>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-orange-500 hover:bg-orange-600 text-white font-black text-xs uppercase tracking-widest rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-orange-500/30 transition-all"
                  >
                    <Icon name="plus-circle" className="w-4 h-4" /> Agregar a Lista
                  </button>
                </div>

                {/* Lista de entradas pendientes */}
                {pendingEntries.length > 0 && (
                  <div className="bg-green-50 border-2 border-green-200 p-3 rounded-2xl space-y-2">
                    <div className="flex justify-between items-center">
                      <p className="text-[9px] font-black text-green-700 uppercase tracking-widest">A Registrar ({pendingEntries.length})</p>
                      <span className="text-[10px] font-black text-green-800 bg-green-200 px-2 py-0.5 rounded-full">+{pendingTotal} pts</span>
                    </div>
                    {pendingEntries.map((e, i) => (
                      <div key={i} className="flex justify-between items-center bg-white px-3 py-2 rounded-xl border border-green-100">
                        <div>
                          <span className="text-xs font-black text-slate-700">{rondaLabel(e.ronda)} · {e.pista === 0 ? 'Global' : `Pista ${e.pista}`}</span>
                          {e.finalTimeMs > 0 && <span className="text-[10px] text-slate-400 ml-2">{e.finalTimeMs / 1000}s</span>}
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-green-700 font-black text-sm">{e.points} pts</span>
                          <button onClick={() => removePending(i)} className="text-red-400 hover:text-red-600 transition-colors">
                            <Icon name="x" className="w-4 h-4" />
                          </button>
                        </div>
                      </div>
                    ))}
                    <div className="bg-slate-800 rounded-xl px-3 py-2 flex justify-between items-center">
                      <span className="text-[10px] font-black text-slate-400 uppercase">Total Final</span>
                      <span className="text-lg font-black text-white">{currentTotal + pendingTotal} pts</span>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button
            onClick={handleSave}
            className={`flex-1 py-3 rounded-xl text-white font-black text-xs uppercase shadow-lg transition-all ${showContingency && isContingencyMode && pendingEntries.length > 0 ? 'bg-orange-600 hover:bg-orange-700 shadow-orange-500/30' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/30'}`}
          >
            {showContingency && isContingencyMode && pendingEntries.length > 0 ? `⚡ Registrar (${pendingEntries.length})` : 'Guardar'}
          </button>
        </div>
...
function ResultadosTab({ teams, currentUser, onShowHistory }) {
  ...
  const getRoundStats = (team, roundFilter) => {
    // Siempre calculamos desde el historial para evitar errores de datos corruptos en team.score
    const rh = Array.isArray(team.history) ? team.history.filter(h =>
      (roundFilter === 'global' ? true : h.ronda === Number(roundFilter)) &&
      h.practice !== true
    ) : [];
  ...
  const sorted = useMemo(() => {
    ...
    // Mostrar si está calificado O si tiene historial en esa ronda (para corregir errores de sincronización)
    if (selRondaView !== 'global') {
      const rd = Number(selRondaView);
      list = list.filter(t =>
        (t.qualifiedRounds || [1]).includes(rd) ||
        (t.history && t.history.some(h => h.ronda === rd))
      );
    }

    return [...list].sort((a, b) => {
      // Prioridad 1: Los descalificados siempre van al final
...
function CompetitionDualOverlay({ teams, questTimer, questTimerActive, toggleQuestTimer, resetQuestTimer, questDuration, lineTimer, lineTimerActive, toggleLineTimer, resetLineTimer, lineDuration, formatTime, onExit, suspenseMode, currentUser, setRankingSuspenseMode, toggleGlobalTimers }) {
  ...
  const getRoundStats = (team, roundFilter, cat) => {
    const rd = roundFilter === 'global' ? null : parseInt(roundFilter);
    const rh = Array.isArray(team.history) ? team.history.filter(h =>
  ...
  const getSorted = (cat) => {
    ...
    // Mostrar si está calificado O tiene historial (contingencia)
    const rd = parseInt(selRondaView);
    const filtered = selRondaView === 'global' ? list : list.filter(t =>
      (t.qualifiedRounds || [1]).includes(rd) ||
      (t.history && t.history.some(h => h.ronda === rd))
    );
